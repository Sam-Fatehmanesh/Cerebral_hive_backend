...
class TestContextAPI(unittest.TestCase):
    ...
    def store_answer(self, question, answer):
        data = {"query": {"query": question}, "answer": {"answer": answer}}
        response = requests.post(
            f"{self.BASE_URL}/store_answer",
    ...
    def post_query(self, question):
        query = {"query": question}
        response = requests.post(
            f"{self.BASE_URL}/post_query",
            headers=self.HEADERS,
            json=query,
            timeout=self.TIMEOUT,
        )
    ...
    def test_context_retrieval(self):
        for question, answer in self.test_data:
            context = self.post_query(question)
            self.assertIn(answer, context)

    def test_context_length(self):
        context = self.post_query("What is the capital of France?")
        self.assertEqual(len(context), 5)

    def test_repeated_query(self):
        context1 = self.post_query("What is the capital of France?")
        context2 = self.post_query("What is the capital of France?")
        self.assertEqual(context1, context2)

    def test_unrelated_query(self):
        context = self.post_query("What is the price of bitcoin?")
        self.assertFalse(any("Paris" in item for item in context))

    def test_long_query(self):
        long_query = "What is the capital of France? " * 100
        context = self.post_query(long_query)
        self.assertTrue(any("Paris" in item for item in context))

    def test_special_characters(self):
        special_query = "What is the capital of France? #$%^&*()!"
        context = self.post_query(special_query)
        self.assertTrue(any("Paris" in item for item in context))

    def test_concurrent_requests(self):
        def make_request(question):
            return self.post_query(question)

        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(make_request, question)
                for question, _ in self.test_data
            ]
